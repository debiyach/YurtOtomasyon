@foreach ($aidats as $item)
    ...
    @if ($item->yatirilacak > 0)
        ...
        <a @if (session()->get('personel'))  @if (session()->get('personel')->tip=='Personel')

            href="{{ Route('personel.pesinOdeme') . '/' . $item->id }}"

        @else
            href="{{ Route('mudur.pesinOdeme') . '/' . $item->id }}" @endif
        @else href="{{ Route('ogrenci.aidatOdeme') . '/' . $item->id }}" @endif class="uzuncard mt-4">
