...
<div class="col-md-6 mt-3 offset-md-3">
    <form method="POST" @if (session()->get('personel')->tip == 'Personel') action="{{ route('personel.personelEklePost') }}"
@else
  action="{{ route('mudur.personelEklePost') }}" @endif enctype="multipart/form-data">
        @csrf
        ...
        <div class="form-group">
            ...
            <input type="tel" class="form-control" name="telNo" value="{{ old('telNo') }}" required
                placeholder="{533-333-3333}" minlength="10" maxlength="10" id="telNo">
        </div>


        <div class="form-group">
            <label for="tcNo">T.C No</label>
            <input type="text" class="form-control" name="tcNo" value="{{ old('tcNo') }}" required min="11"
                maxlength="11" id="tcNo">
        </div>

        <div class="form-group">
            <label for="maas">Maas</label>
            <input type="tel" class="form-control" name="maas" value="{{ old('maas') }}" id="maas">
        </div>

        <div class="form-group">
            <label for="evAdresi">Ev Adresi</label>
            <input type="text" class="form-control" required name="evAdresi" value="{{ old('evAdresi') }}"
                id="evAdresi">
        </div>
